...
describe('RichTextDataFactory', () => {
  beforeEach(() => {
    ...
    (DownloadManager as jest.Mock).mockImplementation(
      (bucket, endpoint, accessKey, secretKey, url, fileName) => ({
  ...
  describe('factorizeRichTextData', () => {
    ...
    test('should use replaceImgSrcWithLocalPath when HTML has content and images are included', async () => {
      const factory = new RichTextDataFactory('<p>Content</p>', '/template', 'project1');

      const emptySpy = jest.spyOn(factory as any, 'checkIfEmptyHtml').mockReturnValue(false as any);
      const replaceSpy = jest
        .spyOn(factory as any, 'replaceImgSrcWithLocalPath')
        .mockResolvedValue(undefined as any);
      const clearSpy = jest.spyOn(factory as any, 'clearImgComponents').mockImplementation(() => {});
    ...
    test('should use clearImgComponents when HTML has content and images are excluded', async () => {
      const factory = new RichTextDataFactory(
        '<p>Content</p>',
        '/template',
        'project1',
  ...
  describe('handleBase64Image', () => {
    ...
    test('should return empty string for non data URL input', async () => {
      const factory = new RichTextDataFactory('<p>Test</p>', '/template', 'project1');
      const data = 'not-a-data-url';

      const result = await (factory as any).handleBase64Image(data);

      expect(result).toBe('');
      expect(DownloadManager).not.toHaveBeenCalled();
    });

    test('should log and return empty string for unsupported image types', async () => {
      const factory = new RichTextDataFactory('<p>Test</p>', '/template', 'project1', 'bucket1');
      const data = 'data:image/bmp;base64,AAAA';

      const result = await (factory as any).handleBase64Image(data);

      expect(result).toBe('');
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Error handling base64 image: data:image/bmp;base64,AAAA')
      );
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Unsupported image type: image/bmp'));
    });
  });
  ...
  describe('checkIfEmptyHtml', () => {
    ...
    test('should detect non-empty HTML when body has text', () => {
      const factory = new RichTextDataFactory('<html><body>Content</body></html>', '/template', 'project1');

      const body = {
        text: jest.fn().mockReturnValue('Some content'),
        find: jest.fn().mockReturnValue({ length: 0 }),
        children: jest.fn().mockReturnValue({
          get: jest.fn().mockReturnValue([]),
        }),
      ...
      const root: any = (selectorOrEl: any) => {
        if (selectorOrEl === 'body') {
          return body;
        }
      ...
      const result = (factory as any).checkIfEmptyHtml();
      expect(result).toBe(false);
    });

    test('should detect non-empty HTML when a child element contains content', () => {
      const factory = new RichTextDataFactory(
        '<html><body><div>Child</div></body></html>',
        '/template',
        'project1'
      );
      ...
      const body = {
        ...
        find: jest.fn().mockReturnValue({ length: 0 }),
        children: jest.fn().mockReturnValue({
          get: jest.fn().mockReturnValue([childEl]),
        }),
      };
      ...
      const root: any = (selectorOrEl: any) => {
        ...
        if (selectorOrEl === childEl) {
          return {
            text: jest.fn().mockReturnValue('Child content'),
            find: jest.fn().mockReturnValue({ length: 0 }),
          };
        }
        throw new Error('Unexpected selector');
  ...
  describe('image helpers', () => {
    test('clearImgComponents should reset image attributes', () => {
      const factory = new RichTextDataFactory('<p>Test</p>', '/template', 'project1');

      const imageNode: any = {
        attrs: { src: 'old.png', style: 'width:10px', width: '10', height: '20' },
      };
      ...
      const root: any = (selectorOrEl: any) => {
        ...
        if (selectorOrEl === imageNode) {
          return {
            removeAttr: (name: string) => {
              delete imageNode.attrs[name];
            },
            attr: (name: string, value?: string) => {
              if (value === undefined) {
                return imageNode.attrs[name];
              }
              imageNode.attrs[name] = value;
            },
          };
        }
        throw new Error('Unexpected selector');
      ...
      (factory as any).clearImgComponents();

      expect(imageNode.attrs.src).toBeUndefined();
      expect(imageNode.attrs.style).toBeUndefined();
      expect(imageNode.attrs.width).toBe('100%');
      expect(imageNode.attrs.height).toBe('auto');
    });

    test('replaceImgSrcWithLocalPath should handle missing, base64 and URL images', async () => {
      const factory = new RichTextDataFactory('<p>Test</p>', '/template', 'project1');

      const imgNoSrc: any = { attrs: {} };
      const imgBase64: any = { attrs: { src: 'data:image/png;base64,AAA' } };
      const imgUrl: any = { attrs: { src: 'https://example.com/image.png' } };

      const images = [imgNoSrc, imgBase64, imgUrl];

      const root: any = (selectorOrEl: any) => {
        if (selectorOrEl === 'img') {
          return images;
        }
        const node = images.find((i) => i === selectorOrEl);
        if (node) {
          return {
            attr: (name: string, value?: string) => {
              if (value === undefined) {
      ...
      const base64Spy = jest
        .spyOn(factory as any, 'handleBase64Image')
        .mockResolvedValue('TempFiles/base64.png');
      const downloadSpy = jest
        .spyOn(factory as any, 'downloadImageAndReturnLocalPath')
        .mockResolvedValue('TempFiles/remote.png');

      await (factory as any).replaceImgSrcWithLocalPath();

      expect(logger.warn).toHaveBeenCalledWith('Image source not found');
      expect(base64Spy).toHaveBeenCalledWith('data:image/png;base64,AAA');
      expect(downloadSpy).toHaveBeenCalledWith('https://example.com/image.png');
      expect(imgBase64.attrs.src).toBe('TempFiles/base64.png');
      expect(imgUrl.attrs.src).toBe('TempFiles/remote.png');
    });
  });
});
